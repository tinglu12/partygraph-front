...
UseGraphProps): UseGraphReturn => {
  ...
  const allTags = useMemo(
    () =>
      Array.from(new Set(events.flatMap((event) => event.tags || []))).map(
        (tag) => ({
          id: `tag-${tag}`,
          name: tag,
          type: "tag",
  ...
  const expandTagNode = async (nodeId: string) => {
    ...
    relatedEvents.forEach((event: EventNode, index: number) => {
      ...
      if (existingNode && existingNode.length === 0) {
        ...
        const _graphEvents = [...graphEvents, event];
        console.log("graphEvents", _graphEvents);
              ...
              .filter(
                (edge) =>
                  (edge.source().id() === nodeId &&
                    edge.target().id() === event.id) ||
                  (edge.source().id() === event.id &&
          ...
          const existingEdges =
            cyRef.current
              ...
              .filter(
                (edge) =>
                  (edge.source().id() === nodeId &&
                    edge.target().id() === tagId) ||
                  (edge.source().id() === tagId &&
                    edge.target().id() === nodeId)
              ) || [];
  ...
  useEffect(() => {
    ...
    cy.on("tap", "node", (evt) => {
      ...
      if (nodeType === "event") {
        ...
        const eventData = graphEvents.find(
          (e: EventNode) => e.id === node.id()
        );
  ...
  useEffect(() => {
    ...
    if (!graphEvents || !graphEdges) {
      // before data is ready
      console.warn("No graph events or edges found");
      return;
    }
    ...
    cyRef.current.add(
      graphEvents.map((event: EventNode) => ({
        group: "nodes",
        data: {
          id: event.id,
          title: event.title,
  ...
  const handleSearch = (query: string) => {
    ...
    //   edges.filter((edge) =>
    //     filtered.some(
    //       (event) => event.id === edge.source || event.id === edge.target
    //     )
    //   )
    // );
  };
