...
export async function searchEventsByVibe(vibeQuery: string): Promise<EventNode[]> {
  try {
    ...
    if (matchingEvents.length > 0) {
      ...
      console.log(`Found ${uniqueEvents.length} events for vibe: "${vibeQuery}"`);
      return uniqueEvents;
    }
...
export async function searchTagCenteredByVibe(vibeQuery: string): Promise<TagCenteredGraphData | null> {
  try {
    ...
    const relatedEvents = sampleEvents.filter(event => 
    ...
    console.log(`Created tag-centered graph with central concept "${centralTag}" and ${relatedEvents.length} events`);
    return graphData;
  } catch (error) {
