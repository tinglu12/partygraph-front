...
export async function searchEventsByVibe(
  ...
  try {
    ...
    if (llamaResult.selectedTags.length > 0 && llamaResult.matchingEvents.length > 0) {
      ...
      console.log(`LLaMA success: ${llamaResult.selectedTags.length} tags, ${llamaResult.matchingEvents.length} events`);
      console.log(`LLaMA selected tags: [${llamaResult.selectedTags.join(', ')}]`);
    ...
    if (legacyResult) {
      ...
      if (matchingEvents.length > 0) {
        ...
        console.log(`Legacy LLaMA found ${uniqueEvents.length} events for: "${vibeQuery}"`);
        return uniqueEvents;
      }
    }
...
function calculateTagSimilarity(query: string, tag: string): number {
  ...
  if (queryLower.includes(tagLower) || tagLower.includes(queryLower)) return 0.9;
  ...
  for (const queryWord of queryWords) {
    for (const tagWord of tagWords) {
      if (queryWord === tagWord || queryWord.includes(tagWord) || tagWord.includes(queryWord)) {
        matchingWords++;
        break;
      }
    }
  ...
  if (matchingWords > 0) {
    return 0.5 + (matchingWords / Math.max(queryWords.length, tagWords.length)) * 0.4;
  }
...
function searchEventsByMultipleTags(query: string): EventNode[] {
  ...
  const allTags = Array.from(new Set(
    sampleEvents.flatMap(event => event.tags || [])
  ));
  ...
  const eventScores = new Map<string, { event: EventNode; score: number; matchingTags: string[] }>();
  ...
  console.log(`Found ${rankedEvents.length} events matching tags [${similarTags.join(', ')}]`);
  return rankedEvents;
}
  ...
  try {
    ...
    try {
      ...
      if (llamaResult.selectedTags.length > 0 && llamaResult.matchingEvents.length > 0) {
        ...
        console.log(`LLaMA tag-centered search successful: [${llamaResult.selectedTags.join(', ')}]`);
        similarTags = llamaResult.selectedTags;
        searchMethod = 'llama';
        
        // Convert LLaMA's EventTypes back to our EventNodes
        const matchingIds = new Set(llamaResult.matchingEvents.map(e => e.title));
        const finalEvents = sampleEvents.filter(event => matchingIds.has(event.title));
        
        if (finalEvents.length > 0) {
          // Use LLaMA's results and tags
          return createTagCenteredGraphFromEvents(finalEvents, similarTags, searchMethod, vibeQuery);
        }
      }
    } catch (llamaError) {
